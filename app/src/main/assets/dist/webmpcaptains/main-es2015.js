...
const APP_ROUTES = _angular_router__WEBPACK_IMPORTED_MODULE_1__["RouterModule"].forRoot(appRoutes);
...
const COMPONENTS_ROUTES = _angular_router__WEBPACK_IMPORTED_MODULE_1__["RouterModule"].forChild(componentsRoutes);
...
let HereMapComponent = class HereMapComponent {
    ...
    // tslint:disable-next-line: max-line-length
    constructor(here, pedidoS, usuarioService, pushNotifications) {
        this.here = here;
    ...
    ngAfterContentInit() {
        ...
        setTimeout(() => {
            ...
            this.map = new H.Map(this.mapElement.nativeElement, this.defaultLayers.vector.normal.map, 
    ...
    notify(texto) {
        ...
        this.pushNotifications.create('webmpcaptains', options).subscribe(res => console.log(res), err => console.log(err));
    }
};
HereMapComponent.ctorParameters = () => [
...
let HomeComponent = class HomeComponent {
    constructor(router, pedidoS, msService) {
        this.router = router;
        this.pedidoS = pedidoS;
...
let InputsFormComponent = class InputsFormComponent {
...
InputsFormComponent.ctorParameters = () => [
...
let LoginComponent = class LoginComponent {
    constructor(userService, router, here) {
        this.userService = userService;
        this.router = router;
        this.here = here;
    }
...
let OnlineComponent = class OnlineComponent {
    constructor(here, pedidoS, usuarioService) {
        this.here = here;
        this.pedidoS = pedidoS;
        this.usuarioService = usuarioService;
        this.online = false;
...
OnlineComponent.ctorParameters = () => [
    { type: src_app_services_heremap_service__WEBPACK_IMPORTED_MODULE_2__["HeremapService"] },
...
const API_KEY = '5cNr9lfPUqqKYFqkhhr_0TUo5LQ8xnowwlh1GlcgbWU';
const URL_HERE = 'https://image.maps.api.here.com';
const URL_REPARTIDOR = 'assets/repartidor.png';
const URL_TIENDA = 'assets/tienda.png';
const URL_PEDIDO = 'assets/pedido.png';
const URL_ENGLISH = 'assets/ingles.jpg';
const URL_SPANISH = 'assets/spain.jpg';
const TIENDA_OLIVA = { latitud: 38.9178068, longitud: -0.1269189 };
const TIENDA_GANDIA = { latitud: 38.9764272, longitud: -0.1909041 };
const TIENDA_TAVERNES = { latitud: 39.0766803, longitud: -0.2868987 };
const INFO_OLIVA = 'Oliva ' + '<br> ' + 'c/ Montaña';
const INFO_GANDIA = 'Gandia ' + '<br> ' + 'c/ Rosales';
const INFO_TAVERNES = 'Tavernes ' + '<br> ' + 'Cami la Dula';
...
let HeremapService = class HeremapService {
    constructor(router, http, deviceService) {
        this.router = router;
        this.http = http;
    ...
    getMarkerTienda() {
        let iconTienda = new H.map.Icon(_config_config__WEBPACK_IMPORTED_MODULE_6__["URL_TIENDA"], { size: { w: 56, h: 56 } });
        let markers = [];
        let marker;
        switch (localStorage.getItem('hub')) {
            case 'oliva':
            ...
            case 'tavernes':
                ...
                break;
            case 'all':
                let markerOliva = new H.map.Marker({ lat: _config_config__WEBPACK_IMPORTED_MODULE_6__["TIENDA_OLIVA"].latitud, lng: _config_config__WEBPACK_IMPORTED_MODULE_6__["TIENDA_OLIVA"].longitud }, { icon: iconTienda });
                let markerGandia = new H.map.Marker({ lat: _config_config__WEBPACK_IMPORTED_MODULE_6__["TIENDA_GANDIA"].latitud, lng: _config_config__WEBPACK_IMPORTED_MODULE_6__["TIENDA_GANDIA"].longitud }, { icon: iconTienda });
                // tslint:disable-next-line: max-line-length
                let markerTavernes = new H.map.Marker({ lat: _config_config__WEBPACK_IMPORTED_MODULE_6__["TIENDA_TAVERNES"].latitud, lng: _config_config__WEBPACK_IMPORTED_MODULE_6__["TIENDA_TAVERNES"].longitud }, { icon: iconTienda });
    ...
    deleteMarker(marker, mapa) {
        if (marker instanceof H.map.Marker) {
            mapa.removeObjects(marker);
        }
    }
    updateRepartidor(lat, lng) {
        let iconRepartidor = new H.map.Icon(_config_config__WEBPACK_IMPORTED_MODULE_6__["URL_REPARTIDOR"], { size: { w: 56, h: 56 } });
